  ...
  <main class="min-h-screen bg-slate-100 px-4 py-8 sm:px-6">
    <div class="mx-auto max-w-7xl">
      <header class="flex flex-col gap-4 border-b border-slate-200 pb-6 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <p class="text-sm font-semibold text-blue-600">Organización</p>
        ...
        <div class="flex items-center gap-4">
          ...
          <button type="button" :disabled="pending" class="rounded-lg bg-blue-700 px-4 py-2 text-sm font-semibold text-white hover:bg-blue-800 disabled:opacity-50" @click="refresh()">
      ...
      <template v-else-if="dashboard">
        <section class="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3" aria-label="Estadísticas del organizador">
          <article v-for="card in cards" :key="card.label" class="relative overflow-hidden rounded-xl border border-slate-200 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
