...
const { data: categories } = await useAsyncData('my-events-categories', () => listCategories())
...
const {
  ...
  error,
  refresh
} = await useAsyncData('my-events-list', () => list({ organizer: authStore.user?.id }))
...
<template>
  <main class="mx-auto max-w-4xl px-4 py-8">
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-2xl font-bold text-slate-900">Mis actividades</h1>
        <p class="mt-1 text-sm text-slate-500">Crea, edita y administra tus actividades.</p>
      </div>
      <button
        type="button"
    ...
    <div v-if="pending" class="mt-8 text-center text-sm text-slate-500">Cargando tus actividades...</div>

    <div v-else-if="error" class="mt-8 rounded-lg border border-red-200 bg-red-50 p-4 text-sm text-red-700">
      No se pudieron cargar tus actividades. Verifica tu conexión e inténtalo de nuevo.
    </div>

    <div v-else-if="!myEvents || myEvents.length === 0" class="mt-8 text-center text-sm text-slate-500">
